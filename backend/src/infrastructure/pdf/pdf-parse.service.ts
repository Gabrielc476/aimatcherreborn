...
export class LibPdfService implements PDFService {
  ...
  async extrairTexto(fileBuffer: Buffer): Promise<string> {
    try {
      this.logger.log(
        `Iniciando extração de PDF com @libpdf/core. Tamanho do buffer: ${fileBuffer.length} bytes`,
      );
      ...
      this.logger.log(
        `Extração com @libpdf/core concluída. Comprimento do texto: ${textoFinal.length}`,
      );
      ...
      if (textoFinal.length < 50) {
        this.logger.log(
          'Texto extraído insuficiente ou vazio. Iniciando fallback com OCR via Gemini...',
        );

        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey) {
          throw new Error(
            'Texto extraído do PDF está vazio e GEMINI_API_KEY não foi configurada para o fallback do OCR.',
          );
        }
        ...
        try {
          ...
          this.logger.warn(
            `Erro no OCR com modelo principal '${modelName}'. Tentando fallback com '${fallbackModel}'... Erro: ${primaryError.message || primaryError}`,
          );
          response = await ai.models.generateContent({
            model: fallbackModel,
            ...ocrPayload,
        ...
        this.logger.log(
          `OCR com Gemini concluído com sucesso. Comprimento do texto extraído: ${textoFinal.length}`,
        );
      }

      if (!textoFinal) {
        throw new Error(
          'Nenhum caractere pôde ser extraído deste PDF via leitor estruturado ou OCR.',
        );
      }
      ...
      this.logger.error(
        `Erro ao extrair texto com LibPDF/Gemini OCR: ${(error as Error).message}`,
      );
      throw new Error(
        `Erro ao extrair texto do PDF: ${(error as Error).message}`,
      );
    }
  }
}
