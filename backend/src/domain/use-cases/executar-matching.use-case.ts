...
export class ExecutarMatchingUseCase {
  ...
  async execute(input: ExecutarMatchingInput, jobId?: string): Promise<Matching> {
    try {
      await this.updateJob(jobId, 'PROCESSANDO', 'carregando_dados', 'Carregando perfil e requisitos da vaga...');
      ...
      await this.updateJob(jobId, 'PROCESSANDO', 'comparando_dados', 'Comparando habilidades e preferências...');

      // 3. Pré-calcula os scores de suporte para a IA
      const scoresSuporte = this.calcularScoresSuporte(usuario, vaga);

      await this.updateJob(jobId, 'PROCESSANDO', 'analise_ia', 'IA analisando afinidades e gerando feedback...');
      ...
      const resultadoAnalise = await this.aiService.analisarCompatibilidade(
        ...
        scoresSuporte,
        async (passo, msg) => {
          await this.updateJob(jobId, 'PROCESSANDO', passo, msg);
        }
      );

      await this.updateJob(jobId, 'PROCESSANDO', 'salvando', 'Salvando resultado final...');
      ...
      const savedMatching = await this.matchingRepository.salvar(matching);

      await this.updateJob(jobId, 'CONCLUIDO', 'finalizado', 'Análise concluída com sucesso!', { matching: savedMatching });

      return savedMatching;
    } catch (err: any) {
      const errorMsg = err instanceof Error ? err.message : 'Erro interno de compatibilidade';
      await this.updateJob(jobId, 'ERRO', 'erro', errorMsg);
      throw err;
    }
  }

  private calcularScoresSuporte(
    usuario: any,
    vaga: any,
  ): { skillScore: number; experienceScore: number; preferenceScore: number } {
    ...
    if (reqSkills.length > 0) {
      ...
      (usuario.curriculoExtraido?.habilidades_tecnicas || []).forEach(
        (h: any) => {
          if (h.nome) candSkillsSet.add(h.nome.toLowerCase().trim());
        },
      );
    ...
    if (tempoMinimo > 0) {
      ...
      const anosCandidatoExtraido =
        usuario.curriculoExtraido?.perfil?.anos_experiencia || 0;

      let maxAnosSkills = 0;
      (usuario.habilidades || []).forEach((h: any) => {
        if (h.anosExperiencia > maxAnosSkills)
          maxAnosSkills = h.anosExperiencia;
      });
      (usuario.curriculoExtraido?.habilidades_tecnicas || []).forEach(
        (h: any) => {
          if (h.anos_experiencia > maxAnosSkills)
            maxAnosSkills = h.anos_experiencia;
        },
      );

      const anosCandidato = Math.max(
        anosCandidatoPerfil,
        anosCandidatoExtraido,
        maxAnosSkills,
      );
      experienceScore = Math.min(
        100,
        Math.round((anosCandidato / tempoMinimo) * 100),
      );
    }
    ...
    if (modalidadeVaga) {
      ...
      (usuario.curriculoExtraido?.preferencias?.modalidades || []).forEach(
        (m: any) => {
          prefsCandidato.add(String(m).toUpperCase().trim());
        },
      );
      ...
      if (prefsCandidato.size > 0) {
        ...
        if (prefsCandidato.has(modVagaUpper)) {
          ...
          if (modVagaUpper === 'HIBRIDO' && prefsCandidato.has('REMOTO')) {
            ...
            modVagaUpper === 'REMOTO' &&
            prefsCandidato.has('HIBRIDO')
          ) {
            preferenceScore = 70;
          } else if (
            modVagaUpper === 'PRESENCIAL' &&
            prefsCandidato.has('HIBRIDO')
          ) {
            preferenceScore = 50;
          } else if (
            modVagaUpper === 'HIBRIDO' &&
            prefsCandidato.has('PRESENCIAL')
          ) {
            preferenceScore = 50;
          } else {
            preferenceScore = 20;
