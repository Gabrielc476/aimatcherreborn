...
export class OtimizarCurriculoUseCase {
  ...
  async execute(input: OtimizarCurriculoInput): Promise<any> {
    ...
    if (input.vagaId) {
      ...
      vagaTitulo =
        input.vagaDescricao.split('\n')[0]?.substring(0, 50) || 'Vaga Externa';
    } else {
      throw new Error(
        'É necessário fornecer o ID da vaga ou a descrição da vaga alvo',
      );
    }
...
`;
    ...
    const resumeData = {
      ...
      titulo_profissional:
        usuario.perfil?.titulo || 'Profissional de Tecnologia',
      email: usuario.email,
    ...
    const titulo =
      input.tituloPersonalizado || `Currículo Otimizado - ${vagaTitulo}`;
