...
export abstract class MatchingRepository {
  ...
  abstract buscarPorUsuario(usuarioId: string, limite: number, pagina: number): Promise<{ total: number; matchings: Matching[] }>;
  abstract buscarPorVaga(vagaId: string, scoreMinimo: number, limite: number, pagina: number): Promise<{ total: number; matchings: Matching[] }>;
